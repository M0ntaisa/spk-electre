...
class Algoritma extends CI_Controller {
    ...
    {
        $cek = $this->db->select('*')->from('tb_alternatif')->get()->num_rows();
    ...
    public function proses()
    {
        ...
        $criteria = $this->Algoritma_model->getBobotKriteria();
         ...
         //-- menghitung nilai Preferensi V
            ...
            $tres_c =  "Threshold c = ".$sigma_c." / ( ".$m." * ( ".$m." - 1 ) ) = ".$threshold_c;
            // echo $threshold_c;
            ...
            $tres_d =  "Threshold d = ".$sigma_d." / ( ".$m." * ( ".$m." - 1 ) ) = ".$threshold_d;
            // echo $threshold_d;
            ...
            for ($i=0; $i < count($klp_ternak) ; $i++) { 
                ...
                $klp_ternak[$i]['Ckl'] = $c[$no];
                $klp_ternak[$i]['Dkl'] = $d[$no];
                $klp_ternak[$i]['C'] = $C[$no];
                $klp_ternak[$i]['D'] = $D[$no];
                $klp_ternak[$i]['F'] = $F[$no];
                $klp_ternak[$i]['G'] = $G[$no];
                $klp_ternak[$i]['E'] = $E[$no];
                for ($j=0; $j < count($eval_klp_ternak[$i]); $j++) { 
                    $klp_ternak[$i]['evaluasi'][$j]['X'] = $X_beta[$i][$j]['X'];
                    $klp_ternak[$i]['evaluasi'][$j]['R'] = $R_beta[$i][$j]['R'];
                    $klp_ternak[$i]['evaluasi'][$j]['V'] = $V_beta[$i][$j]['V'];
                }$no++;
            }
